...
public interface ReportRepository extends JpaRepository<Report, Long> {
    ...
    @Query(value = "SELECT reportItems.reportId , reportItems.checkItemContent , reportItems.checkItemCreatedTime , remainsQuestion.rectificationTime , remainsQuestion.measure , remainsQuestion.result , reportItems.reportItemId, reportItems.reportItemLevel, reportItems.checkItemId, reportItems.checkMainItemId FROM ( SELECT a.id AS reportItemId , a.report_id AS reportId , a.jhi_level AS reportItemLevel, b.content AS checkItemContent , b.created_date AS checkItemCreatedTime , b.id AS checkItemId, b.parent_id AS checkMainItemId FROM report_items a LEFT JOIN check_item b ON a.check_item_id = b.id WHERE a.report_id = ?1) reportItems LEFT JOIN( SELECT a.report_items_id , b.rectification_time AS rectificationTime , b.measure , b.result FROM remains_question a RIGHT JOIN rectification b ON a.rectification_id = b.id) AS remainsQuestion ON reportItems.reportItemId = remainsQuestion.report_items_id ORDER BY reportItemId ASC", nativeQuery = true)
    List<Object[]> findAllReportOverview(Long reportId);

    @Query(value = "SELECT reportItems.reportId,  reportItems.checkItemContent,  reportItems.checkItemCreatedTime,  remainsQuestion.rectificationTime,  remainsQuestion.measure,  remainsQuestion.result,  reportItems.reportItemId,  reportItems.reportItemLevel,  reportItems.checkMainItemId FROM  (   SELECT  a.id AS reportItemId,  a.report_id AS reportId,  a.jhi_level AS reportItemLevel,  (  SELECT   content  FROM   check_item  WHERE   id = b.parent_id  ) AS checkItemContent,  b.created_date AS checkItemCreatedTime,  b.id AS checkItemId,  b.parent_id AS checkMainItemId   FROM  report_items a   LEFT JOIN check_item b ON a.check_item_id = b.id   WHERE  a.report_id = ?1  ) reportItems LEFT JOIN (  SELECT   a.report_items_id,   b.rectification_time AS rectificationTime,   b.measure,   b.result  FROM   remains_question a  RIGHT JOIN rectification b ON a.rectification_id = b.id ) AS remainsQuestion ON reportItems.reportItemId = remainsQuestion.report_items_id ORDER BY  checkMainItemId ASC", nativeQuery = true)
    List<Object[]> findAllReportOverview4MainCheckItem(Long reportId);
